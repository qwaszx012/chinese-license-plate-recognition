...
%自动弹出提示框读入图像
 [filename,filepath]=uigetfile('.jpg','choose a vehicle image(muist be standard chinese license plate with blue background and white character)');% 直接自动读入%
 file=strcat(filepath,filename); %strcat函数：连接字符串；把filepath的字符串与filename的连接，即路径/文件名
 I=imread(file);
 figure('name','original image'),imshow(I);title('original image')
 %图像增强
 % h=ones(5,5)/25; %过滤器h
...
%线性灰度变换
 ...
 figure('name','after grayscale'),subplot(1,2,1),imshow(I1);title('after grayscale');
 subplot(1,2,2),imhist(I1);title('histogram after grayscale');
%进行中值滤波
 I1=medfilt2(I1);
 figure,imshow(I1);title('median filter');
 imwrite(I1,'median filter.jpg');

%边缘检测：sobel,roberts,canny,prewitt等
 I2=edge(I1,'roberts',0.25,'both'); %边缘检测算法，强度小于阈值0.15的边缘被省略掉,'both'两个方向检测（缺省默认）
 % I2=edge(I1,'roberts',0.15,'both'); %边缘检测算法，强度小于阈值0.15的边缘被省略掉,'both'两个方向检测（缺省默认）
figure('name','edge detection'),imshow(I2);title('robert edge detection') 
imwrite(I2,'robert edge detection.jpg');
 se=[1;1;1];
 I3=imerode(I2,se);% 腐蚀Imerode(X,SE).其中X是待处理的图像，SE是结构元素对象
figure('name','image after corrosion'),imshow(I3);title('image after corrosion');
imwrite(I3,'image after corrosion.jpg');
 se=strel('rectangle',[20,20]);% 25X25的矩形 strel???
 I4=imclose(I3,se);% 用25*25的矩形对图像进行闭运算(先膨胀后腐蚀)有平滑边界作用
 figure('name','smoothing process'),imshow(I4);title('image after smoothing');
 imwrite(I4,'image after smoothing.jpg');
 I5=bwareaopen(I4,1000);% 从二进制图像中移除所有少于2000像素的连接对象，消失的是连续的白色像素数量少于2000的字符
 figure('name','remove small object'),imshow(I5);title('remove snall object');
  imwrite(I5,'remove snall object.jpg');
 ...
 figure('name','Extract license plate from image'),%subplot(1,2,2),
 imshow(dw),title('Extract license plate from image')
 imwrite(dw,'dw.jpg');
 imwrite(dw,'Extract license plate from image.jpg');
 ...
 a=imread('dw.jpg');
 b=rgb2gray(a);
 imwrite(b,'1.gray scale image.jpg');
figure('name','image processing');subplot(3,2,1),imshow(b),title('1.grayscale image')
...
%线性灰度变换
 ...
 subplot(3,2,2),imshow(b);title('2.Grayscale image after linear gray scale processing');

%进行二值化处理
 d=im2bw(b,0.4);%将灰度图像进行二值化处理
 imwrite(d,'2.binary image.jpg');
 subplot(3,2,3),imshow(d),title('3.binary image');%显示二值化图像

%进行中值滤波
 d=medfilt2(d);
 imwrite(d,'4.after Median filter.jpg');
 subplot(3,2,4),imshow(d);title('4.after Median filter');
...
%se=eye(7);
 %imopen(d,se);
 imwrite(d,'5.after corrosion or expansion.jpg');
 subplot(3,2,5),imshow(d),title('5.after corrosion or expansion')
...
while j~=n   %%%%% 什么原理？？？
 ...
 % 分割出第七个字符
 [word7,d]=getword(d);
 figure('name','character after segmentation');subplot(5,7,1),imshow(word1),title('1');
...
figure('name','character after Normalization');subplot(5,7,15),imshow(word1),title('11');
 ...
 figure(10),subplot(5,7,1:7),imshow(dw),title('1：license plate location'),
 xlabel({'2: Character segmentation'}); %'',
...
xlabel(['3：Character recognition save to excel :', Code],'Color','b');
xlswrite('d:\lpr.xls',Code,'A1:M1'); %存入excel表格，表格需要存储在桌面时请更改路径，并且以管理员权限打开matlab
% handles = guihandles(gcf);              %若没有excel则创
%set(handles.text1,'string',num2str(Code)); %本来用于gui可视化的。立
